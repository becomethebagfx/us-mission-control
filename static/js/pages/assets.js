            ...
            container.innerHTML = `<div class="page-enter">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    <div class="mc-card text-center">
                        <div class="text-2xl font-bold text-navy">${st.total || items.length || 0}</div>
                        <div class="text-xs text-gray-500 mt-1">Total Assets</div>
                    </div>
                    <div class="mc-card text-center">
                        <div class="text-2xl font-bold text-navy">${st.templates || 0}</div>
                        <div class="text-xs text-gray-500 mt-1">Templates</div>
                    </div>
                    <div class="mc-card text-center">
                        <div class="text-2xl font-bold text-navy">${st.social_cards || 0}</div>
                        <div class="text-xs text-gray-500 mt-1">Social Cards</div>
                    </div>
                    <div class="mc-card text-center">
                        <div class="text-2xl font-bold text-navy">${st.showcases || 0}</div>
                        <div class="text-xs text-gray-500 mt-1">Showcases</div>
                    </div>
                </div>
                ...
                <div class="mc-card">
                    <div class="flex items-center justify-between mb-6">
                        <h2 class="text-lg font-semibold text-navy flex items-center gap-2">
                            <i data-lucide="image" class="w-5 h-5"></i> Asset Library
                        </h2>
                    </div>
                        ...
                        <div class="border border-gray-100 rounded-xl overflow-hidden hover:border-gray-200 hover:shadow-sm transition-all">
                            ...
                            <div class="p-4">
                                ...
                                <div class="flex items-center gap-2 mt-2">
                                    <span class="mc-badge mc-badge-info">${this._esc(a.type || 'asset')}</span>
                                    <span class="mc-badge">${this._esc(a.company || '')}</span>
                    ...
                    </div>` : `<div class="empty-state"><i data-lucide="image" class="empty-state-icon"></i><p class="empty-state-title">No assets generated</p><p class="empty-state-text">Generate visual assets from templates to see them here.</p></div>`}
                </div>
            </div>`;
