...
window.PageHome = {
    ...
    async render(company) {
        ...
        try {
            ...
            const [summary, funnel, gbp] = await Promise.all([
                API.dashboard.summary(company),
                API.reactivation.funnel(company),
                API.gbp.insights(),
            ]);

            container.innerHTML = this._buildHTML(summary, funnel, gbp);
    ...
    _postQueue(li) {
        // Build a placeholder list; the summary endpoint gives counts,
        // but we try to show the last 5 scheduled posts if available.
        // If summary contains scheduled_posts array, use it; otherwise show count.
        const posts = li.scheduled_posts || [];
        ...
        if (hasDetail) {
            listHTML = posts.slice(0, 5).map(p => `
                <div class="flex items-start gap-3 py-2.5 border-b border-gray-50 last:border-0">
                    <span class="company-dot company-dot-${this._esc(p.company_slug || 'us-framing')} mt-1.5 flex-shrink-0"></span>
                    <div class="min-w-0 flex-1">
                        <p class="text-sm text-navy font-medium truncate">${this._esc(p.title || 'Untitled Post')}</p>
                        <p class="text-xs text-gray-400 mt-0.5">${this._esc(p.scheduled_date || '')}</p>
                    </div>
    ...
    _contentPipeline(co) {
        const stages = [
            { label: 'Draft', count: co.draft || co.drafts || 0, badge: 'badge-draft', icon: 'file-edit' },
            { label: 'Review', count: co.review || co.in_review || 0, badge: 'badge-review', icon: 'eye' },
            { label: 'Approved', count: co.approved || 0, badge: 'badge-approved', icon: 'check-circle' },
            { label: 'Published', count: co.published || 0, badge: 'badge-published', icon: 'globe' },
        ];
                                ...
                                <div>
                                    ...
                                    <div class="w-full bg-gray-100 rounded-full h-1.5">
                                        <div class="h-1.5 rounded-full transition-all duration-500"
                                             style="width: ${pct}%; background-color: var(--bar-color, #4A90D9)"></div>
                                    </div>
                                </div>`;
                        }).join('')}
                    </div>
    ...
    _leadFunnel(funnel) {
        ...
        return `
            <div class="mc-card">
                ...
                <div class="mc-card-body">
                    <canvas id="home-funnel-chart" height="180"></canvas>
    ...
    _gbpInsights(totals) {
        const metrics = [
            { label: 'Views', value: this._num(totals.views), icon: 'eye' },
            { label: 'Clicks', value: this._num(totals.clicks), icon: 'mouse-pointer-click' },
            { label: 'Calls', value: this._num(totals.calls), icon: 'phone' },
            { label: 'Directions', value: this._num(totals.directions), icon: 'map-pin' },
        ];
                        ...
                        ${metrics.map(m => `
                            <div class="flex items-center gap-3">
                                <div class="w-9 h-9 rounded-lg bg-gray-50 flex items-center justify-center">
                                    <i data-lucide="${m.icon}" class="w-4 h-4 text-gray-400"></i>
                                </div>
                                <div>
                                    <div class="text-lg font-semibold text-navy">${m.value}</div>
